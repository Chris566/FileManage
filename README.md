...
- **深色 / 浅色主题**、**中文 / English 界面**，设置（含上次目录）自动持久化
...
2. 启用重命名 / 分类整理，按需调整模板、前缀、替换规则、分类规则
...
| 文件 | 路径 | 说明 |
|---|---|---|
| rules.json | `%AppData%\FileManage\rules.json` | 分类规则（可导入导出） |
| settings.json | `%AppData%\FileManage\settings.json` | 主题、语言、上次目录 |
